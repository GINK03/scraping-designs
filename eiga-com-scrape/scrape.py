...
def html(url): 
  try:
    ...
    try:
      r = requests.get(url, headers=headers, proxies=dict(http='socks5://localhost:9050', https='socks5://localhost:9050'))
...
def main():
  ...
  while urls != set():
    print(urls)
    nextUrls = set()
    with concurrent.futures.ProcessPoolExecutor(max_workers=32*4) as executor:

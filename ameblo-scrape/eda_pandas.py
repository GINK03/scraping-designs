...
if '--init' in sys.argv:
    args = [(idx, 'profile', path) for idx, path in enumerate(sorted(Path('./tmp').glob('ameblo_jsons_profile_*.csv')))]
    with PPE(max_workers=8) as exe:
        exe.map(pmap_profile, args)

    args = [(idx, 'content', path) for idx, path in enumerate(sorted(Path('./tmp').glob('ameblo_jsons_content_*.csv')))]
    with PPE(max_workers=8) as exe:
        exe.map(pmap_content, args)

if '--join' in sys.argv:
    dfProfile = [pd.read_pickle(path) for path in Path('./tmp').glob('profile_*.pkl')]
    dfProfile = pd.concat(dfProfile, axis=0)

    dfContent = [pd.read_pickle(path) for path in Path('./tmp').glob('content_*.pkl')]
...
if '--agg' in sys.argv:
    df = pd.read_csv('tmp/ameblo_user.csv')
